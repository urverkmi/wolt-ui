...
export function CityView() {
  ...
  return (
    <div className="fixed inset-0 bg-white overflow-hidden" style={{ fontFamily: "'Baloo Tammudu 2', sans-serif" }}>
      ...
      {/* Right side buttons */}
      <div className="absolute top-[72px] right-[19px] z-20 flex flex-col gap-3">
        {/* Wrapped button */}
        <button
      ...
      {/* DEBUG: should show a red square in the middle */}
      <div className="absolute inset-0 z-10">
        {displayedLocations.map((location) => {
          if (!location.mapPosition) return null;
          const isQuestTarget = questTargetLocations.some(q => q.id === location.id);

          return (
            <div key={location.id} className="absolute" style={{ top: location.mapPosition.top, left: location.mapPosition.left }}>
              <button
              ...
              >
                ...
                {location.visitCount && location.visitCount > 1 && (
                  <div className="absolute -bottom-1 -right-1 bg-white rounded-full shadow-[0px_0px_0.5px_0.5px_#d8d8d8] w-[16px] h-[16px] flex items-center justify-center z-20">
                    <span className="text-[12px] text-black font-bold">{location.visitCount}</span>
                  </div>
                )}
              </button>
              ...
              {location.visited && (
                <div className="absolute top-[-16px] left-1/2 transform -translate-x-1/2 bg-white rounded-full shadow-[0px_0px_0.5px_0.5px_#d8d8d8] px-3 h-[21px] flex items-center justify-center whitespace-nowrap">
                  <span className="text-[12px] text-black">{location.name}</span>
                </div>
              )}
            </div>
          );
      ...
      {/* Bottom Sheet */}
      <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-[20px] z-30" style={{ height: '150px' }}>
        {/* Handle */}
        ...
        <div className="px-[18px] pt-4">
          ...
          <button
            onClick={() => setExpandedSection(expandedSection === 'quests' ? null : 'quests')}
            className="w-full bg-white opacity-20 rounded-[16px] shadow-[0px_2px_4px_1px_rgba(0,0,0,0.2)] h-[58px] relative"
          />
          <div className="absolute left-[18px] right-[18px] flex items-center justify-between h-[58px]" style={{ top: 'calc(5px + 16px + 12px)' }}>
            <div className="flex items-center gap-4">
              <img src={badgeIcon} alt="Badge" className="w-[39px] h-[39px] ml-[10px]" />
              <span className="text-[20px] text-black font-bold">Active Quests ({activeQuests.length})</span>
            </div>
            <button onClick={() => setExpandedSection(expandedSection === 'quests' ? null : 'quests')}>
              {expandedSection === 'quests' ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
            </button>
          </div>
        </div>
      </div>
          ...
          <div className="p-6">
            ...
            <div className="space-y-3">
              {activeQuests.map((quest) => {
                return (
                  <div key={quest.id} className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <h3 className="mb-1 text-[#141414]">{quest.title}</h3>
                        <div className="text-xs text-[#009de0]">🎁 {quest.reward}</div>
                      </div>
                      <div className="text-sm text-gray-600">{quest.progress}/{quest.total}</div>
                    </div>
                    ...
                    <button
                      onClick={() => {
                        setHighlightedQuestId(highlightedQuestId === quest.id ? null : quest.id);
                    ...
                    >
                      {highlightedQuestId === quest.id ? 'Hide on Map' : 'Show Qualified Places on Map'}
                    </button>
                  </div>
                );
              })}
      ...
      {selectedLocation && (
        <div className="fixed inset-0 bg-black/60 flex items-end z-50" onClick={() => setSelectedLocation(null)}>
          <div className="bg-white w-full rounded-t-3xl p-6 max-h-[80vh] overflow-y-auto z-50" onClick={e => e.stopPropagation()}>
            {(() => {
                ...
                <>
                  ...
                  <div className="mb-4">
                    <div className="flex items-start justify-between mb-2 z-50">
                      <h3 className="text-2xl text-[#141414]">{location.name}</h3>
                      {location.visitCount && (
                  ...
                  <div className="flex space-x-2">
                    <button 
                      ...
                      className="flex-1 bg-[#009de0] text-white py-3 rounded-xl">Order Now</button>
          ...
          <div className="min-h-screen p-6 flex flex-col">
            ...
            <div className="flex-1 flex flex-col justify-center space-y-6">
              <div className="text-center mb-8">
                <h1 className="text-4xl mb-2 text-white">Your {cityName}</h1>
              ...
              <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-6 shadow-xl space-y-6">
                <div className="text-center">
                  <div className="text-7xl mb-2 text-[#141414]">{wrappedStats.totalVisits}</div>
                ...
                <div className="text-center">
                  <div className="text-sm text-gray-500 mb-2">Your favorite spot</div>
                  <div className="text-3xl mb-2 text-[#141414]">{wrappedStats.topPlace?.name}</div>
                  <div className="text-[#009de0]">{wrappedStats.topPlace?.visitCount} visits</div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="text-2xl mb-1 text-[#141414]">{wrappedStats.totalPhotos}</div>
